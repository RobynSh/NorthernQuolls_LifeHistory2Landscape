...
Prior to this step, we merged ALA and NatureMap data sets and removed unnecessary columns and duplicated records (which left only one ALA record, all of the others were already represented in the NatureMap data base). Missing coordinate info was filtered out prior to this script. Rasters were also derived/prepared/manipulated prior to this step. For example, all rasters have been aggregated to the same resolution based on the most biologically appropriate metric (mean, median, min, max, etc.), and cropped/masked to the same extent (the Pilbara IBRA region, with some coverage into surrounding regions). For details on raster manipulation/creation see the Supplementary material for the paper (Appendix S2-S3).
...
- Generating background points using the bias layer created in step 1 (*Dh_FinalBackgroundCoords.csv*)
- Running MaxEnt with the package SDMtune (looping through for each raster set), including splitting data for training (with cross validation), tuning, testing, running the default model, dropping correlated variables, tuning hyperparameters, dropping variables with low importance and running the final model. 
